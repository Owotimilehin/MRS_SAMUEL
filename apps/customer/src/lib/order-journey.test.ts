...
describe("deriveJourney", () => {
  it("live track when a rider exists, in Lagos, immediate", () => {
    ...
    expect(j.steps.map((s) => s.key)).toEqual([
      "placed",
      "paid",
      "preparing",
      "on_the_way",
  ...
  it("coordinated track when outside Lagos, overriding schedule", () => {
    const j = deriveJourney({
      ...base,
      delivery_state: "Oyo",
      scheduled_delivery_at: "2026-06-22T11:00:00Z",
    });
  ...
  it("out_for_delivery marks the OTW step current and preparing done", () => {
    const j = deriveJourney({
      ...base,
      delivery: { status: "in_transit" },
      out_for_delivery_at: "2026-06-21T13:20:00Z",
    });
  ...
  it("delivered marks all done", () => {
    const j = deriveJourney({
      ...
      out_for_delivery_at: "x",
      delivered_at: "2026-06-21T13:40:00Z",
    });
