...
/**
 * Level-2 local mock end-to-end for the OPay migration.
 *
 * Drives the REAL customer checkout UI (catches render crashes), then
 * simulates the server-to-server callback that OPay would send (there's no
 * real OPay in mock mode), and verifies the order flips to paid + stock drops.
 *
 * Prereqs: api :3001, customer :3002, DB seeded, OPay creds empty (mock mode).
 */
const CUSTOMER = "http://localhost:3002";
const API = "http://localhost:3001";

test("OPay mock checkout: order → callback → paid + stock decremented", async ({ page }) => {
  const errors: string[] = [];
  ...
  await page.getByLabel(/Full name/i).fill("OPay Test Buyer");
  ...
  console.log(`[opay-test] ${radioCount} courier option(s); selected the last`);
  ...
  await page.route("**/v1/public/orders", async (route) => {
    ...
    try {
      ...
      else console.log(`[opay-test] order POST ${orderStatus} body:`, text);
    } catch {
      /* leave orderBody null — assertion below will surface it */
    }
    await route.fulfill({ response: resp, body: text });
  });
  // Block the post-pay redirect to the mock OPay URL — we simulate the callback
  ...
  console.log(`[opay-test] order ${orderNo} total ₦${captured.data.total_ngn}`);
  console.log(`[opay-test] mock checkout url: ${captured.data.payment.authorization_url}`);
  ...
  console.log(`[opay-test] pre-callback status: ${preBody.data.status}/${preBody.data.payment_status}`);
  expect(preBody.data.payment_status).not.toBe("paid");

  // 4. Simulate OPay's server-to-server callback. In mock mode the webhook's
  //    queryOpayOrder() returns SUCCESS, so this drives the real paid path.
  const cb = await page.request.post(`${API}/v1/webhooks/opay`, {
    headers: { "content-type": "application/json" },
    data: { payload: { reference: orderNo, status: "SUCCESS" } },
  });
  ...
  console.log(`[opay-test] post-callback status: ${postBody.data.status}/${postBody.data.payment_status}`);
  ...
  const replay = await page.request.post(`${API}/v1/webhooks/opay`, {
    headers: { "content-type": "application/json" },
    data: { payload: { reference: orderNo, status: "SUCCESS" } },
  });
